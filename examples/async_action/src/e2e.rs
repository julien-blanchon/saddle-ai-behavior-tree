use bevy::prelude::*;
use saddle_ai_behavior_tree::BehaviorTreeSystems;
use saddle_bevy_e2e::action::Action;

use crate::scenarios;

pub struct AsyncActionE2EPlugin;

impl Plugin for AsyncActionE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(saddle_bevy_e2e::E2EPlugin);
        app.configure_sets(
            Update,
            saddle_bevy_e2e::E2ESet.before(BehaviorTreeSystems::Prepare),
        );
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[behavior_tree_async_action:e2e] Unknown scenario '{name}'. Available: {:?}",
